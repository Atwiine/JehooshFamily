...
import com.example.jehooshfamily.ui.Adapters.ViewResponseDrawingAnswersAdapter;
...
public class AnswersDrawing extends AppCompatActivity {

    Urls urls;
    RecyclerView recyclerView_answer_drawing;
    TextView no_answer_drawing;
    List<AnswersDrawing_Model> mData;
    SessionManager sessionManager;
    ViewResponseDrawingAnswersAdapter amswersAdapter;
    String getId, names, question_id;
    TextView qn_id, qn, qn_date, open_folder_show_doc;
    SpinKitView spinKitView;
    String fileFinalName;
    String fileName = "Drawing-answers";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_answers_drawing);
...
// attaching the pasted on extras
        ...
        no_answer_drawing = findViewById(R.id.no_answer_draw);
        ...
        //handle recyclerview
        recyclerView_answer_drawing = findViewById(R.id.recyclerView_answer_drawing);
        mData = new ArrayList<>();
        recyclerView_answer_drawing.setLayoutManager(new LinearLayoutManager(this));
        amswersAdapter = new ViewResponseDrawingAnswersAdapter(this, mData);
        recyclerView_answer_drawing.setAdapter(amswersAdapter);
...
//    }
    ...
    private void updateAnswer() {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, urls.UPDATEANSWER_DRAW,
    ...
    private void getResponseDrawing() {
        final ProgressDialog dialog = new ProgressDialog(this);
        dialog.setMessage("Please wait as we fetch the answers for this question");

        spinKitView.setVisibility(View.GONE);
        StringRequest stringRequest = new StringRequest(Request.Method.POST, urls.RESPONSE_DRAW_ANSWER_URL,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        try {
                            ...
                            if (tips.length() == 0) {
                                spinKitView.setVisibility(View.GONE);
                                no_answer_drawing.setVisibility(View.VISIBLE);
                            } else {
                                for (int i = 0; i < tips.length(); i++) {
                                    ...
                                    String photo = jsonObject.getString("photo").trim();
                                    ...
                                    String date_submission = jsonObject.getString("date").trim();
                                    String explanation = jsonObject.getString("explanation").trim();


                                    AnswersDrawing_Model markets_model = new AnswersDrawing_Model(id,
                                            photo,//this is going to be the photo
                                            from_who,
                                            userid,
                                            sent_question,
                                            sent_qn_id,
                                            boss_name,
                                            boss_id,
                                            date_submission,explanation);
                                    mData.add(markets_model);
                                    dialog.dismiss();
                                    spinKitView.setVisibility(View.GONE);
                                    no_answer_drawing.setVisibility(View.GONE);

                                }
                                amswersAdapter = new ViewResponseDrawingAnswersAdapter(AnswersDrawing.this, mData);
                                recyclerView_answer_drawing.setAdapter(amswersAdapter);
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                            ...
                            spinKitView.setVisibility(View.GONE);
                            no_answer_drawing.setVisibility(View.VISIBLE);
                            Toast.makeText(AnswersDrawing.this, " something went wrong, please try again", Toast.LENGTH_SHORT).show();
                        }
                    }
                }, new Response.ErrorListener() {
            @Override
            public void onErrorResponse(VolleyError error) {
                ...
                no_answer_drawing.setVisibility(View.VISIBLE);
//
                Toast.makeText(AnswersDrawing.this, "could not load your answers, please check your connection and try again", Toast.LENGTH_LONG).show();
            }
        }) {
    ...
    public void goback(View view) {
        startActivity(new Intent(AnswersDrawing.this, ResponseQnDrawing.class));
    }
    ...
    private void exportEssayAnswers() {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, urls.EXPORT_DRAWANSWERS,
                response -> {
                    try {
                        Log.i("tagconvertstr", "[" + response + "]");
                        JSONArray tips = new JSONArray(response);
...
//            show_error.setText(error.toString());
    ...
    public void exportFile() {
        try {
            ...
            Uri downloadUri = Uri.parse(urls.download_draw_answers);
    ...
    public void open_intents(View view) {
        ...
        try {
            FileOpen.openFile(AnswersDrawing.this, myFile);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
